...
vec3 p(vec2 xy, float m, float t) {
  ...
  for(int i=1;i<45;i++) {
    vec2 newp=p + t*0.02;
    newp.x+=1.2/float(i)*sin(float(i)*p.y+t/fluid_speed+0.6*float(i)) + 1.0;
    newp.y+=0.6/float(i)*sin(float(i)*p.x+t/fluid_speed+0.3*float(i+10)) - 0.5;
    p=newp;
  }
  return vec3(
    color_intensity*sin(0.3*p.x)+color_intensity,
    color_intensity*sin(8.0*p.y)+color_intensity,
...
void main() {
  ...
  gl_FragColor=vec4(vec3(col.y + col.z + 0.15), 1.0);
}
